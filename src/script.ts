...
/*
const startdatetime = "2008-12-13T19:10:00Z"; 
const enddatetime = "2020-12-20T21:00:00Z";  
    ...
    for (const musikhjalpenYear of musikhjalpen) {
        ...
        if (playlistId) {
            for (const chunk of chunkedUriResult) {
                ...
                await sleep(10000);  // Pausa i 10 sekunder
            }
        }
    }
}
...
async function searchSongs(token: string, baseEndPoint: string, songs: string[], year: string): Promise<string[]> {
    ...
    for (const song of songs) {
        ...
        //const endPoint: string = `${baseEndPoint}/search?q=track${encodeURIComponent(title + " ")}artist${encodeURIComponent(artist)}&type=track&market=SE&limit=3`;
...
function checkSongCorrectness(artist: string, title: string, searchArtist: string, searchTitle: string): number {
    ...
    const artistEdited = artist.split('(')[0].trim(); // SR har ofta med (SV) eller liknande
...
function selectTrackURI(topTracks: TrackObject[], artist: string, title: string) : string | undefined{
    ...
    //console.log(`Song: ${joinArtists(topTracks[0].artists)} - ${topTracks[0].name} : ${topCorrectness}`)
    ...
    for (let i = 1; i < topTracks.length; i++) {
        if (topTracks[i].artists) {
            ...
            //console.log(`Song: ${joinArtists(topTracks[i].artists)} - ${topTracks[i].name} : ${currentCorrectness}`)
...
// This function edits the artist and title from SR since they have some irregularities
function SREditSearchString(song: string, year: string) : string[] {
    ...
    // Special cases that have been discovered through error searching
...
async function fetchSongsSR(currentTime: Date, showStopTime: string) {
    ...
    .then(response => {
        ...
        // Extract the descriptions into an array
...
// SPOTIFY TYPES
// Fyll i rätt typer i underkategorierna när tid, viktigaste är att tracks är rätt
